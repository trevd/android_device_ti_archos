...
HMC5843Sensor::HMC5843Sensor()
    : SensorBase(NULL, "hmc5843"),
      mEnabled(0),
      mInputReader(4),
      mHasPendingEvent(false)
{
    ...
    if (data_fd) {
        strcpy(input_sysfs_path, "/sys/bus/i2c/drivers/hmc5843/4-001e/");
...
int HMC5843Sensor::setInitialState() {
...
int HMC5843Sensor::enable(int32_t, int en) {
    int flags = en ? 1 : 0;
    if (flags != mEnabled) {
        int fd;
...
bool HMC5843Sensor::hasPendingEvents() const {
    return mHasPendingEvent;
}

int HMC5843Sensor::setDelay(int32_t handle, int64_t delay_ns)
{
    int fd;
...
int HMC5843Sensor::readEvents(sensors_event_t* data, int count)
{
    if (count < 1)
        return -EINVAL;
