...
#define LOG_TAG "Archos Tablet PowerHAL"
#include <utils/Log.h>
...
struct archos_tablet_power_module {
...
static void archos_tablet_power_init(struct power_module *module)
{
    struct archos_tablet_power_module *archos_tablet =
                                   (struct archos_tablet_power_module *) module;
    int tmp;
    ...
    archos_tablet->inited = 1;
}

static int boostpulse_open(struct archos_tablet_power_module *archos_tablet)
{
    char buf[80];

    pthread_mutex_lock(&archos_tablet->lock);

    if (archos_tablet->boostpulse_fd < 0) {
        archos_tablet->boostpulse_fd = open(BOOSTPULSE_PATH, O_WRONLY);

        if (archos_tablet->boostpulse_fd < 0) {
            if (!archos_tablet->boostpulse_warned) {
                strerror_r(errno, buf, sizeof(buf));
                ALOGE("Error opening %s: %s\n", BOOSTPULSE_PATH, buf);
                archos_tablet->boostpulse_warned = 1;
            }
        }
    }

    pthread_mutex_unlock(&archos_tablet->lock);
    return archos_tablet->boostpulse_fd;
}

static void archos_tablet_power_set_interactive(struct power_module *module,
                                               int on)
{
    struct archos_tablet_power_module *archos_tablet =
                                   (struct archos_tablet_power_module *) module;

    if (!archos_tablet->inited) {
        return;
    }
...
static void archos_tablet_power_hint(struct power_module *module,
                                    power_hint_t hint, void *data)
{
    ...
    case POWER_HINT_INTERACTION:
        if (boostpulse_open(archos_tablet) >= 0) {
            len = write(archos_tablet->boostpulse_fd, "1", 1);
...
struct archos_tablet_power_module HAL_MODULE_INFO_SYM = {
    .base = {
        .common = {
            ...
            .name = "Archos Tablet Power HAL",
            .author = "The Android Open Source Project",
            .methods = &power_module_methods,
        },

       .init = archos_tablet_power_init,
       .setInteractive = archos_tablet_power_set_interactive,
       .powerHint = archos_tablet_power_hint,
    },
